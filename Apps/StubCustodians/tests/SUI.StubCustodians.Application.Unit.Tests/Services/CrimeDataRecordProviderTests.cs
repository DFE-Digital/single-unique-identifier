...
{
    public class CrimeDataRecordProviderTests
    {
        private readonly string _tempDir;

        public CrimeDataRecordProviderTests()
        {
        ...
        {
            var provider = new CrimeDataRecordProvider(_tempDir);
        ...
        public void GetRecordForSui_ShouldThrow_WhenJsonIsInvalid()
        {
            ...
            string providerId = "MockCrimeDataProvider";

            string providerFolder = Path.Combine(_tempDir, providerId);
            Directory.CreateDirectory(providerFolder);

            string filePath = Path.Combine(providerFolder, $"{sui}_CrimeDataRecordV1.json");
        ...
        public void GetRecordForSui_ShouldReturnEnvelope_WhenJsonIsValid()
        {
            ...
            Assert.Equal(
                new Uri("https://schemas.example.gov.uk/sui/CrimeDataRecordV1.json"),
                result.SchemaUri
            );
        }
    }
