...
public class SearchEndpointTests
{
    ...
    {
        // Arrange
        var data = new StartSearchRequest { Suid = ValidNhsId };
        var orchestratorData = new SearchOrchestratorParameters(data.Suid, _context.InvocationId);
        var httpRequestData = MockHttpRequestData.Create(data);
        _client
            .ScheduleNewOrchestrationInstanceAsync(nameof(SearchOrchestrator), orchestratorData)
            .Returns("test-job-id");
    ...
    public async Task ShouldReturn202_WithSearchJobData_WhenSuccessful()
    {
        ...
        var httpRequestData = MockHttpRequestData.Create(data);
        _client
            .ScheduleNewOrchestrationInstanceAsync("SearchOrchestrator", orchestratorData)
            .Returns("test-job-id");
    ...
    {
        // Arrange
        var data = new StartSearchRequest { Suid = InvalidNhsId };
        var httpRequestData = MockHttpRequestData.Create(data);
