...
public class QueryProvidersFunctionTests
{
    ...
    public QueryProvidersFunctionTests()
    {
        ...
        _function = new QueryProvidersFunction(_mockLogger, _httpClientFactory, _encryptionService, _maskUrlService);
    }
    ...
    public async Task QueryProvider_ReturnsEmptyList_WhenEncryptionIsNull()
    {
        ...
        var input = new QueryProviderInput("client-id-1", "job-id-1", "invocation-id", "1234567890123456", provider);

        // Act
        // Assert
        await Assert.ThrowsAsync<InvalidOperationException>(() => _function.QueryProvider(_mockContext, input));
    ...
    {
        // Arrange
        var provider = MockProvider(encryption: new EncryptionDefinition { Key = "test-key-1", KeyId = "key-1", Algorithm = "AES" });

        var input = new QueryProviderInput("client-id-1", "job-id-1", "invocation-id", "1234567890123456", provider);

        _encryptionService.EncryptNhsToPersonId(Arg.Any<string>(), Arg.Any<EncryptionDefinition>())
    ...
    {
        // arrange
        var provider = MockProvider(encryption: new EncryptionDefinition { Key = "test-key", KeyId = "key-1", Algorithm = "AES" });
        var input = new QueryProviderInput("client-id-1", "job-id-1", "invocation-id", "1234567890123456", provider);
        ...
        _encryptionService.EncryptNhsToPersonId(Arg.Any<string>(), Arg.Any<EncryptionDefinition>()).Returns(Result<string>.Ok("encrypted-person-id"));

        var successClient = new HttpClient(new MockHttpMessageHandler((req, ct) =>
        ...
        _maskUrlService.CreateAsync(Arg.Any<List<SearchResultItem>>(), input, Arg.Any<CancellationToken>())
            .Returns(maskedItems);
    ...
    public async Task QueryProvider_ReturnsEmptyList_WhenUnsuccessful()
    {
        // Arrange
        var provider = MockProvider(encryption: new EncryptionDefinition { Key = "test-key", KeyId = "key-1", Algorithm = "AES" });
        var input = new QueryProviderInput("client-id", "job-id", "invocation-id", "12345", provider);

        _encryptionService.EncryptNhsToPersonId(Arg.Any<string>(), Arg.Any<EncryptionDefinition>())
            .Returns(Result<string>.Ok("encrypted-person-id"));

        var failedClient = new HttpClient(new MockHttpMessageHandler((req, ct) =>
