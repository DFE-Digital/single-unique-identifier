...
public sealed class AuthTokenFunction
{
    ...
    public AuthTokenFunction()
    {
        _tokenLifetimeMinutes = Store.Value.DefaultTokenLifetimeMinutes > 0
            ? Store.Value.DefaultTokenLifetimeMinutes
            : 60;
    }

    [Function("AuthToken")]
    [OpenApiOperation(operationId: "authToken", tags: new[] { "Auth" }, Summary = "Issue a sandbox bearer token using client credentials")]
    [OpenApiRequestBody("application/json", typeof(AuthTokenRequest), Required = true)]
    [OpenApiRequestBody("application/x-www-form-urlencoded", typeof(AuthTokenFormRequest), Required = false)]
    ...
    public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Function, "post", Route = "v1/auth/token")] HttpRequestData req,
        FunctionContext context)
    {
        ...
        if (body is null)
        {
            return await ProblemResponse(req, HttpStatusCode.BadRequest, "Invalid request", "Missing or malformed token request.");
        }

        if (string.IsNullOrWhiteSpace(body.ClientId) || string.IsNullOrWhiteSpace(body.ClientSecret))
        {
            return await ProblemResponse(req, HttpStatusCode.BadRequest, "Invalid request", "Missing client_id or client_secret.");
        }
        ...
        {
            return await ProblemResponse(req, HttpStatusCode.Unauthorized, "Unauthorised", "Invalid client credentials.");
        }
        ...
        await res.WriteAsJsonAsync(new AuthTokenResponse
        {
            AccessToken = token,
            TokenType = "Bearer",
    ...
    private static async Task<AuthTokenRequest?> ReadAuthRequest(HttpRequestData req)
    {
        ...
        if (contentType.StartsWith("application/x-www-form-urlencoded", StringComparison.OrdinalIgnoreCase) || looksLikeForm)
        {
        ...
        try
        {
            return JsonSerializer.Deserialize<AuthTokenRequest>(raw, new JsonSerializerOptions
            {
    ...
    private static (string? ClientId, string? ClientSecret) TryReadBasicClientCredentials(HttpRequestData req)
    {
    ...
    private static AuthStore LoadAuthStore()
    {
        ...
        var store = JsonSerializer.Deserialize<AuthStore>(json, new JsonSerializerOptions
        {
            PropertyNameCaseInsensitive = true
        });
        ...
        if (string.IsNullOrWhiteSpace(store.Issuer) ||
            string.IsNullOrWhiteSpace(store.Audience) ||
            string.IsNullOrWhiteSpace(store.SigningKey))
        {
            throw new InvalidOperationException("Auth store file is missing issuer, audience, or signingKey.");
        }

        store.Clients ??= new List<AuthClient>();
