...
public sealed class ScopeEnforcementService : IFunctionsWorkerMiddleware
{
    ...
    public async Task Invoke(FunctionContext context, FunctionExecutionDelegate next)
    {
        ...
        if (!req.Headers.TryGetValues("Authorization", out var authHeaders))
        {
            context.GetInvocationResult().Value =
                await ProblemResponse(req, HttpStatusCode.Unauthorized, "Unauthorised", "Missing Authorization header.");
            return;
        }
        ...
        {
            context.GetInvocationResult().Value =
                await ProblemResponse(req, HttpStatusCode.Unauthorized, "Unauthorised", "Missing or invalid bearer token.");
            return;
        }
        ...
        {
            context.GetInvocationResult().Value =
                await ProblemResponse(req, HttpStatusCode.Unauthorized, "Unauthorised", "Missing bearer token.");
            return;
        }
        ...
        {
            context.GetInvocationResult().Value =
                await ProblemResponse(req, HttpStatusCode.Unauthorized, "Unauthorised", ex.Message);
            return;
        }
        catch
        {
            context.GetInvocationResult().Value =
                await ProblemResponse(req, HttpStatusCode.Unauthorized, "Unauthorised", "Invalid bearer token.");
            return;
        }
        ...
        {
            context.GetInvocationResult().Value =
                await ProblemResponse(req, HttpStatusCode.Unauthorized, "Unauthorised", "Insufficient scope for this operation.");
            return;
        }
    ...
    private static bool HasAnyRequiredScope(AuthContext caller, IReadOnlyList<string> requiredScopes)
    {
